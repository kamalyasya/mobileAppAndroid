...
const extensions = {
    ...
    async activateRecentWebview() {
        ...
        throw new Error(`No webview has been detected after ${timer.getDuration().asMilliSeconds.toFixed(0)}ms. ` +
    ...
    async listWebFrames(useUrl = true) {
        ...
        const maxRetriesCount = lodash_1.default.isInteger(this.opts.webviewConnectRetries)
            ...
            : undefined;
        this.log.debug(`About to select a web application with ${support_1.util.pluralize('retry', maxRetriesCount ?? 20, true)} ` +
        ...
        try {
            const pageArray = await ( /** @type {RemoteDebugger} */(this.remote)).selectApp(shouldFilterByUrl ? this.getCurrentUrl() : undefined, maxRetriesCount, this.opts.ignoreAboutBlankUrl);
